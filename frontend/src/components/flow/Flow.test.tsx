...
describe("Flow", () => {
  it("renders exactly one primary, in the footer, on every page", async () => {
    const { container, findByRole, getByRole } = renderFlow(["meet", "agent", "compute"]);
    ...
    expect(primary(container)).toHaveTextContent("Next");
    // Back has arrived, and the cards are radios rather than buttons.
  ...
  it("disables the primary with the missing prerequisite as its title", async () => {
    ...
    expect(primary(container)).toHaveAttribute("title", "Choose an agent to continue");
  ...
  it("goes back one page, and the page shows the answer it was given", async () => {
    ...
    expect(primary(container)).toHaveAttribute("title", "Answer the question to continue");
  ...
  it("offers no Back on the first page of the first run", async () => {
    const { findByRole, queryByRole } = renderFlow(["meet", "agent", "compute"]);
    await findByRole("heading", { level: 1, name: "Meet flyco" });
  ...
  it("reports a refused primary above the footer and keeps the primary", async () => {
    ...
    const { container, findByRole, findByLabelText, getByRole } = renderFlow(["agent"], {
      answers: { agent: "codex", agentRoute: "api-key" },
      position: 2,
    });
    ...
    expect(getByRole("heading", { level: 1, name: "Paste your API key" })).toBeInTheDocument();
  });
});
